...
- **BUG CRITIQUE** : Créé `renderEP2()` — l'onglet EP2 (CAP IFCA) était vide car la fonction n'existait pas
- **BUG CRITIQUE** : Créé `renderEP3()` — l'onglet EP3 (CAP IFCA) était vide car la fonction n'existait pas
- **BUG CRITIQUE** : Créé `renderModules()` — l'onglet Modules (2nde TNE) était vide car la fonction n'existait pas
- Étendu `compBlock()` pour gérer les critères EP2 (`CRIT_EP2`), EP3 (`CRIT_EP3`) et modules (pas de critères)
- Ajouté boutons contexte dans tab-ep2 : Atelier / PFMP 1 / PFMP 2 / Dossier
- Ajouté boutons situation dans tab-ep3 : Mise en service (A) / Maintenance (B) / Communication (C)
- Corrigé `_detectFormationFromClasse()` : utilise `iwState` en priorité (fixe filtre TP)
- UX : `min-height:44px` sur tous les `.btn` (touch-friendly mobile)
- Cache Service Worker : v7.6 → v7.7
- VERSION_FRONT : 7.6.3 → 7.7.0
