...
### Nouvelle base stable depuis tt-ia
- Création tt-ia-v3 : copie intégrale de tt-ia (v7.6.3) comme point de départ stable
- Référentiels CAP IFCA / Bac Pro MFER / TNE : validés dans formations.json (v5.3)
- Tokens sécurisés : nouveau format AELV001D (préfixe typé + numéro séquentiel + lettre de contrôle)
  - Élève : AELV001X, Tuteur : TELV001X
  - Remplace l'ancien format aléatoire (Math.random)
  - Appliqué dans first-setup.html et inerweb_prof.html
- Taille boutons : minimum 44×44px (WCAG 2.5.5 accessibilité tactile)
  - mobile.css : règle globale + btn-niveau, btn-parse, btn-photo
  - security.css : lock-btn-primary
